...
### 認知負荷を上げない聞き方（2026-09-16 owner decision）

聞き取りの質そのものがこのrepoの価値を左右する。取得側は次を守る。

- 一度に聞く質問は1つだけ。複数項目（いつ・どこで・きっかけ・直後どうなったか等）を
  箇条書きで並べて一度に要求しない。
- `appraisal`、`context.uncertainty`、`observed_facts`のようなschema語彙をそのまま
  質問文に出さない。本人には自然な言葉で聞き、構造化は聞き取り側の内部処理とする。
- 「具体的な1回分にしてください」のように、答える前から型を強制しない。まず自由に
  話してもらい、Event／trigger／outcome等への分解は回答を受けてから取得側が行う。
- 深掘りが必要な場合も、不足項目を列挙して埋めさせるのではなく、会話の流れに沿って
  次の1問だけを聞く。
- 本人が「細かい」「面倒」等の負荷を示す反応をしたら、直ちに質問密度を下げる。これは
  聞き取り継続可否のシグナルであり、無視して同じ聞き方を繰り返さない。
